...
const eventSessionSchema = zod.object({
  id: zod.string(),
  title: zod.string().trim().transform((val) => val.replace(/\r?\n/g, "")),
  day: zod.string().regex(/^\d{2}\/\d{2}\/\d{4}$/),
  start: zod.string().regex(/^\d{2}:\d{2}$/),
...
export type EventSession = zod.infer<typeof eventSessionSchema>;

export function parseEventSessions(rawSessions: unknown[]) {
  const validSessions: EventSession[] = [];
  const invalidSessions: Array<{ session: unknown; error: string }> = [];

  for (const rawSession of rawSessions) {
    const validated = eventSessionSchema.safeParse(rawSession);
    if (!validated.success) {
      invalidSessions.push({
        session: rawSession,
        error: validated.error.message,
